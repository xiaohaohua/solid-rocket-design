...
def get_rspeed(lamda_L,p,ratio,m):
    dx=L/m
    speed=np.zeros(m)#内部使用，内部定义
    for j in range(m):
        location=j*dx
        lamda=get_lamda(location,lamda_L)
        rlamda=get_rlamda(lamda)
        speed[j]=math.pow(p*rlamda,n)*a*ratio
    return speed
...
def update(pold,eold,lamda_Lold,ratioold,m,Vgold,dt):
    rspeedold=get_rspeed(lamda_Lold,pold,ratioold,m)
    tup=get_eupdate(eold,dt,rspeedold,m)
    enew=tup[0]
    Abnew=tup[1]
    Apnew=tup[2]
    tup2=get_parametre(Abnew,Apnew)
    rationew=tup2[2]
    lamda_Lnew=tup2[3]
    mbold=get_mb(eold,rspeedold,m)
    mdold=get_md(pold,lamda_Lold)
    pnewm=pold+R/M*Tf*(mbold-mdold)/Vgold*dt
    rspeednew=get_rspeed(lamda_Lnew,pnewm,rationew,m)
    mbnew=get_mb(enew,rspeednew,m)
    Vgnew=get_Vg(Vgold,mbold,dt)
    mdnew=get_md(pnewm,lamda_Lnew)
    return enew,Abnew,Apnew,rationew,lamda_Lnew,rspeednew,Vgnew,mbnew,mdnew,pnewm
